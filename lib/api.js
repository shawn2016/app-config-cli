...
async function uploadOtherFile(req, res) {
  try {
    ...
    // 保存文件（保持原文件名）
    const filePath = path.join(otherDir, file.originalname);
...
async function deleteOtherFile(req, res) {
  try {
    ...
    const appConfigDir = getAppConfigDir();
    const filePath = path.join(appConfigDir, folderName, 'other', filename);
...
async function convertAABToAPKInternal(file, alias) {
  try {
    ...
    // 检查 bundletool-all.jar 是否存在（位于 lib 目录）
    const bundletoolPath = path.join(__dirname, 'bundletool-all.jar');
    if (!(await fs.pathExists(bundletoolPath))) {
      throw new Error(`bundletool-all.jar 文件不存在，请确保文件位于: ${bundletoolPath}`);
    }
...
async function convertAABToAPK(req, res) {
  try {
    ...
    if (!(await fs.pathExists(bundletoolPath))) {
      return res.status(404).json({ 
        success: false,
        error: `bundletool-all.jar 文件不存在，请确保文件位于: ${bundletoolPath}` 
      });
    }
