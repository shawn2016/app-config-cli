...
const IOS_SPLASH_WIDTH = 1125;
const IOS_SPLASH_HEIGHT = 2436;
const ANDROID_SPLASH_WIDTH = 1080;
const ANDROID_SPLASH_HEIGHT = 1882;
    ...
    for (const dir of dirs) {
      ...
      if (stat.isDirectory()) {
        ...
        if (await fs.pathExists(indexPath)) {
          ...
          let splashscreenIosStyle = '';
          let splashscreenAndroidStyle = '';
          let iosSplashExists = false;
          let androidSplashExists = false;
          try {
            ...
            splashscreenIosStyle = parsedConfig.splashscreenIosStyle || '';
            splashscreenAndroidStyle = parsedConfig.splashscreenAndroidStyle || '';
            ...
            // 检查启动图是否存在
            iosSplashExists = await fs.pathExists(path.join(configPath, 'ios启动图.png'));
            androidSplashExists = await fs.pathExists(path.join(configPath, 'android启动图.png'));
          } catch (err) {
          ...
          configs.push({
            ...
            splashscreenIosStyle: splashscreenIosStyle,
            splashscreenAndroidStyle: splashscreenAndroidStyle,
            iosSplashExists: iosSplashExists,
            androidSplashExists: androidSplashExists,
...
function parseConfigContent(content) {
  ...
  // 解析 splashscreen（可选）
  const splashscreenMatch = content.match(/splashscreen:\s*\{([\s\S]*?)\}/);
  if (splashscreenMatch) {
    const splashStr = splashscreenMatch[1];
    const iosStyleMatch = splashStr.match(/iosStyle:\s*['"]([^'"]+)['"]/);
    const androidStyleMatch = splashStr.match(/androidStyle:\s*['"]([^'"]+)['"]/);
    config.splashscreenIosStyle = iosStyleMatch ? iosStyleMatch[1] : '';
    config.splashscreenAndroidStyle = androidStyleMatch ? androidStyleMatch[1] : '';
  } else {
    config.splashscreenIosStyle = '';
    config.splashscreenAndroidStyle = '';
  }
...
// 上传 iOS 启动图 (storyboard)
async function uploadIOSSplash(req, res) {
  try {
    const { alias } = req.params;
    const file = req.file;

    if (!file) {
      return res.status(400).json({ error: '请选择要上传的 iOS 启动图' });
    }

    if (file.mimetype !== 'image/png') {
      return res.status(400).json({ error: 'iOS 启动图必须是 PNG 格式' });
    }
    ...
    try {
      ...
      if (metadata.width !== IOS_SPLASH_WIDTH || metadata.height !== IOS_SPLASH_HEIGHT) {
        return res.status(400).json({
          error: `iOS 启动图尺寸必须是 ${IOS_SPLASH_WIDTH}x${IOS_SPLASH_HEIGHT}，当前尺寸：${metadata.width}x${metadata.height}`
        });
      }
    } catch (err) {
      if (file.size > 10 * 1024 * 1024) {
        return res.status(400).json({ error: 'iOS 启动图文件过大，请确保小于 10MB' });
      }
    }
    ...
    const appConfigDir = getAppConfigDir();
    const brandDir = path.join(appConfigDir, folderName);
    const splashPath = path.join(brandDir, 'ios启动图.png');
    ...
    if (await fs.pathExists(splashPath)) {
      await fs.remove(splashPath);
    }

    await fs.writeFile(splashPath, file.buffer);

    res.json({
      success: true,
      message: 'iOS 启动图上传成功',
      url: `/appConfig/${folderName}/ios启动图.png`
    });
  } catch (error) {
    console.error('上传 iOS 启动图错误:', error);
    res.status(500).json({ error: error.message });
  }
}

// 上传 Android 启动图 (default)
async function uploadAndroidSplash(req, res) {
  try {
    const { alias } = req.params;
    const file = req.file;

    if (!file) {
      return res.status(400).json({ error: '请选择要上传的 Android 启动图' });
    }

    if (file.mimetype !== 'image/png') {
      return res.status(400).json({ error: 'Android 启动图必须是 PNG 格式' });
    }
    ...
    try {
      const sharp = require('sharp');
      const metadata = await sharp(file.buffer).metadata();
      if (metadata.width !== ANDROID_SPLASH_WIDTH || metadata.height !== ANDROID_SPLASH_HEIGHT) {
        return res.status(400).json({
          error: `Android 启动图尺寸必须是 ${ANDROID_SPLASH_WIDTH}x${ANDROID_SPLASH_HEIGHT}，当前尺寸：${metadata.width}x${metadata.height}`
        });
      }
    } catch (err) {
      if (file.size > 10 * 1024 * 1024) {
        return res.status(400).json({ error: 'Android 启动图文件过大，请确保小于 10MB' });
      }
    }
    ...
    const appConfigDir = getAppConfigDir();
    const brandDir = path.join(appConfigDir, folderName);
    const splashPath = path.join(brandDir, 'android启动图.png');
    ...
    res.json({
      success: true,
      message: 'Android 启动图上传成功',
      url: `/appConfig/${folderName}/android启动图.png`
    });
  } catch (error) {
    console.error('上传 Android 启动图错误:', error);
    res.status(500).json({ error: error.message });
  }
}
...
// 获取 iOS 启动图状态
async function getIOSSplash(req, res) {
  try {
    const { alias } = req.params;
    const folderName = await findFolderByAlias(alias);
    if (!folderName) {
    ...
    const appConfigDir = getAppConfigDir();
    const splashPath = path.join(appConfigDir, folderName, 'ios启动图.png');
    const exists = await fs.pathExists(splashPath);
    res.json({
      exists,
      url: exists ? `/appConfig/${folderName}/ios启动图.png` : ''
    });
  } catch (error) {
    console.error('获取 iOS 启动图错误:', error);
    res.status(500).json({ error: error.message });
  }
}

// 获取 Android 启动图状态
async function getAndroidSplash(req, res) {
  try {
    const { alias } = req.params;
    const folderName = await findFolderByAlias(alias);
    if (!folderName) {
    ...
    const appConfigDir = getAppConfigDir();
    const splashPath = path.join(appConfigDir, folderName, 'android启动图.png');
    const exists = await fs.pathExists(splashPath);
    res.json({
      exists,
      url: exists ? `/appConfig/${folderName}/android启动图.png` : ''
    });
  } catch (error) {
    console.error('获取 Android 启动图错误:', error);
    res.status(500).json({ error: error.message });
  }
}
