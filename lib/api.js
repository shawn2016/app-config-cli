...
async function cloudBuild(req, res) {
  try {
    const { alias } = req.params;
    const { platform, environment, operation, userVersionDesc, noIncrement } = req.body;
    
    if (!platform || !environment) {
      return res.status(400).json({ 
        success: false,
        error: '缺少必填参数: platform, environment' 
      });
    }
    ...
    const env = {
      ...
      OPERATION: operation || 'cloudbuild',
      USER_VERSION_DESC: userVersionDesc || '',
      NO_INCREMENT: noIncrement ? 'true' : 'false'
    };

    console.log(`开始执行云打包，品牌: ${folderName}，平台: ${platform}，环境: ${environment}`);
    console.log(`操作类型: ${operation || 'cloudbuild'}`);

    // 使用 spawn 异步执行命令，实时返回输出
    const childProcess = spawn('npm', ['run', `cloud:build:${platform}:${environment}`], {
      cwd: projectPath,
      env: env,
      shell: true,
