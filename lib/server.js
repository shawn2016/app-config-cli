...
const { getConfigs, createConfig, getConfigByAlias, checkConfigFiles, checkCloudBuildRequirements, generateKeystore, getKeystoreInfo, importConfig, updateConfig, deleteConfig, uploadLogo, deleteLogo, getLogo, uploadIOSSplash, uploadAndroidSplash, getIOSSplash, getAndroidSplash, uploadP12, deleteP12, getP12, uploadMobileprovision, deleteMobileprovision, getMobileprovision, uploadOtherFile, deleteOtherFile, getOtherFiles, generateUnipushCloudFunction, generateAppLinks, getGitBranches, cloudBuild, cancelCloudBuild, updateVersion, convertAABToAPK, deleteDownloadedAPK, uploadToPGYER, testPushNotification } = require('./api');
...
const upload = multer({
  ...
  limits: { fileSize: 500 * 1024 * 1024 } // 500MB，支持 AAB 文件上传
});
...
// 静态文件服务（downloads 目录，用于下载转换后的 APK 文件）
...
// 启动图相关路由
app.post('/api/configs/:alias/splash/ios', upload.single('iosSplash'), uploadIOSSplash);
app.post('/api/configs/:alias/splash/android', upload.single('androidSplash'), uploadAndroidSplash);
app.get('/api/configs/:alias/splash/ios', getIOSSplash);
app.get('/api/configs/:alias/splash/android', getAndroidSplash);

// Other 文件相关路由
app.post('/api/configs/:alias/other', upload.single('file'), uploadOtherFile);
app.delete('/api/configs/:alias/other/:filename', deleteOtherFile);
app.get('/api/configs/:alias/other', getOtherFiles);

// Keystore 信息路由（放在 /api/configs/:alias 之前，避免路由冲突）
app.get('/api/configs/:alias/keystore', getKeystoreInfo);

// 生成 unipush 云函数路由（全局操作）
app.post('/api/generate-unipush', generateUnipushCloudFunction);
...
// 获取 Git 分支列表路由
app.get('/api/git/branches', getGitBranches);
...
// AAB 转 APK 路由（放在 /api/configs/:alias 之前，避免路由冲突）
app.post('/api/configs/:alias/convert-aab-to-apk', upload.single('aab'), convertAABToAPK);

// 删除下载的 APK 文件路由
app.delete('/api/downloads/:filename', deleteDownloadedAPK);

// 上传到蒲公英路由
app.post('/api/upload-pgyer', upload.single('file'), uploadToPGYER);

// 测试消息推送路由（放在 /api/configs/:alias 之前，避免路由冲突）
app.post('/api/configs/:alias/test-push', testPushNotification);

// 通用配置路由（放在最后）
app.get('/api/configs/:alias', getConfigByAlias);
app.get('/api/configs/:alias/check-files', checkConfigFiles);
app.get('/api/configs/:alias/check-cloud-build', checkCloudBuildRequirements);
